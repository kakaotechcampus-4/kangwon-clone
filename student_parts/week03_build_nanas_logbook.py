...
class SaveStructuredRequestInput(StructuredRequest):
    ...
    def unwrap_legacy_payload(cls, value: Any) -> Any:
        ...
        if isinstance(value, dict) and isinstance(value.get("structured_request"), dict):
            return value["structured_request"] #안쪽 dict 반환
        return value
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    #여러 입력 형태 -> SaveStructuredRequestInput 하나로 통일 후 반환 (멘토님 Q1 관련)
...
def save_structured_request_payload(
    ...
    *,
    ...
    #request를 SQLite에 저장하고 싶을 때 쓰는 helper 함수
    normalized = _save_input_from(request)
    resolved_store = store or _store()
    return resolved_store.save_structured_request(normalized.model_dump())
...
def _delete_saved_schedules(
    *,
    ...
    no_condition = (
        not schedule_ids and not date and not title and not start_time
        and not time_unspecified and not delete_all
    )
    ...
    if no_condition:
        return {"ok": False, "deleted_count": 0, "filters": filters, "deleted": [], "reason": "삭제 조건이 없습니다"}
...
def delete_saved_schedules_dict(
    ...
    resolved_store = app_store or _store()
...
def personal_update_saved_schedule(
    ...
    if result is None: #schedule_id가 없으면 에러 대신 None
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        저장 일정 수정 요청("일정 시간/날짜/제목 바꿔줘")은 personal_update_saved_schedule을 쓴다.
        바꾸지 않을 필드는 인자를 비워두면(None) 된다.
        삭제 요청("이 일정 지워줘")은 먼저 personal_list_saved_schedules로 후보를 확인한 뒤,
        personal_delete_saved_schedules에 schedule_ids나 date/title 같은 명시적 조건을 넘겨서 삭제한다.
        조건 없이 전부 지우는 delete_all은 사용자가 "전부/모두 지워줘"처럼 명확히 전체 삭제를 요청했을 때만 쓴다.
        후보가 여러 개 검색됐는데 사용자가 어떤 일정인지 명확히 밝히지 않았으면,
        바로 삭제하지 말고 후보 목록(제목/날짜/시간)을 보여주며 어떤 일정을 지울지 먼저 확인한다.
        """,
    ]
