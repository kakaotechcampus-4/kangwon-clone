...
SQLITE_MEMORY_PROMPT = (
    "일정/할 일/알림은 메모리가 아닌 앱 SQLite DB에 영구 저장된다. "
    "저장된 기록에 대한 질문은 기억으로 답하지 말고 반드시 조회 tool을 호출해 DB 기준으로 답한다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "저장 요청: extract_schedule_request로 구조화한 뒤, 결과의 structured_request 필드 값을 그대로 save_structured_request 인자로 전달한다. 원문이나 wrapper JSON을 통째로 넘기지 않는다. "
    "조회 요청: personal_list_saved_schedules를 호출하고, 날짜가 명확하면 date_from/date_to로 좁힌다. "
    "수정/삭제 요청: 먼저 personal_list_saved_schedules로 schedule_id를 확인한 뒤 personal_update_saved_schedule 또는 personal_delete_saved_schedules를 호출한다. "
    "delete_all=True는 사용자가 전체 삭제를 명시했을 때만 쓴다."
)
...
def personal_list_saved_schedules(
    ...
    request_kind = kind or "personal_schedule"
    schedules = _store().list_schedules(limit=limit, kind=request_kind, date_from=date_from, date_to=date_to)
    filters = {"kind": request_kind, "date_from": date_from, "date_to": date_to, "limit": limit}
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "Week 2의 'SQLite 저장을 하지 않는다'와 '일정 생성 요청일 때만 tool을 호출한다'는 지시는 Week 3부터 해제한다. "
        "이번 주차 범위는 구조화 → SQLite 저장 → 조회/수정/삭제까지이며 이외에 작업은 하지 않는다. ",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
    ]
