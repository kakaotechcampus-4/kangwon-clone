...
SQLITE_MEMORY_PROMPT = "SQLite에 저장된 일정/할 일/알림을 새 대화에서도 조회할 수 있습니다. "

# TODO: 자연어 구조화 → SQLite 저장과 조회/수정/삭제 tool 호출 순서를 안내하는 규칙을 작성하세요.
WEEK03_TOOL_CALL_PROMPT = (
    "사용자가 일정/할 일/알림을 저장해달라고 요청하면 먼저 extract_schedule_request를 호출해 구조화한 뒤, save_structured_request로 SQLite에 저장합니다."
    "'내 일정 보여줘' 같은 조회 요청은 personal_list_saved_schedules를 호출합니다."
    "일정 수정 요청은 personal_update_saved_schedule를 호출합니다"
    "일정 삭제 요청은 personal_delete_saved_schedules를 호출합니다."
)
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    if isinstance(value, str):
        try:
            ...
            value = extract_structured_request(value).model_dump()
...
def _delete_saved_schedules(
    *,
    ...
    if not delete_all and not any([schedule_ids, date, title, start_time, time_unspecified]):
        return {"ok": False, "reason": "삭제 조건이 없습니다.", "filters": filters, "deleted_count": 0, "deleted": []}
    ...
    return {"ok": True, "filters": filters, "deleted_count": len(deleted), "deleted": deleted}
...
def structured_request_from_week01_schedule(schedule: dict[str, Any]) -> SaveStructuredRequestInput:
    ...
    week03_input = SaveStructuredRequestInput(
...
def personal_list_saved_schedules(
    ...
    filter_kind = kind or "personal_schedule"
    schedules = _store().list_schedules(limit=limit, kind=filter_kind, date_from=date_from, date_to=date_to)

    # TODO: filters와 schedules를 포함한 JSON 문자열을 반환하세요.
    return json_payload(tool_result("personal_list_saved_schedules", filters={"limit": limit, "kind": filter_kind, "date_from": date_from, "date_to": date_to}, schedules=schedules))
...
def personal_update_saved_schedule(
    ...
    if not schedule_id:
        return json_payload(tool_result(
            "personal_update_saved_schedule", ok=False, reason="schedule_id가 없습니다.",
        ))
    ...
    if result is None:
        return json_payload(tool_result(
            "personal_update_saved_schedule", ok=False, reason="schedule_id를 찾을 수 없습니다.",
        ))
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        "Week 3의 범위는 자연어 요청을 구조화해 SQLite에 저장하고, 저장된 일정/할 일/알림을 조회·수정·삭제하는 것까지다.",
        "외부 멤버 일정 조율이나 RAG 검색은 이번 주차 범위 밖이다."
    ]
