...
def save_structured_request_payload(
    ...
    *,
    ...
    request_input = _save_input_from(request)
    saved = (store or _store()).save_structured_request(request_input.model_dump(exclude_none=True))
...
def personal_create_schedule(
    ...
    sqlite_save = _store().save_structured_request(request.model_dump(exclude_none=True))
...
def personal_update_saved_schedule(
    ...
    if updated is None:
        return json_payload(tool_result(
            ...
            ok=False,
            schedule_id=schedule_id,
            error="해당 schedule_id의 일정을 찾을 수 없습니다."
        ))
    
    # 2. 정상적으로 업데이트가 이루어진 경우
    return json_payload(tool_result(
        "personal_update_saved_schedule",
