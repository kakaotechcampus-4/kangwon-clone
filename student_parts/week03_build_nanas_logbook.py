...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 일정/할 일/알림은 대화가 끝나도 앱 SQLite DB에 남아."
)

WEEK03_TOOL_CALL_PROMPT = (
    """
자연어 요청은 extract_schedule_request로 먼저 구조화하고, 그 결과 필드를 save_structured_request 인자로 전달해서 저장해.
조회는 personal_list_saved_schedules 또는 list_saved_requests를 사용해.
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    if isinstance(value, SaveStructuredRequestInput): # 이미 원하는 타입이면 그대로 사용
        return value
    if isinstance(value, StructuredRequest): # Week2 StructuredRequest는 저장용 스키마로 변환
        return SaveStructuredRequestInput.model_validate(value.model_dump())
    if isinstance(value, dict): # dict도 저장용 스키마 검증 후 사용
        return SaveStructuredRequestInput.model_validate(value)
    if isinstance(value, str): # 문자열이면 JSON인지 확인
        try:
    ...
    raise RuntimeError("지원하지 않는 입력 형식입니다.")
...
def save_structured_request_payload(
    ...
    *,
    ...
    saved = active_store.save_structured_request(validated.model_dump())
    ...
    *,
    ...
    if not delete_all and not any([schedule_ids, date, title, start_time, time_unspecified]):
        return tool_result(
            ...
            filters=filters,
            deleted=[],
            error="삭제 조건이 없습니다. schedule_ids나 필터를 지정하거나 delete_all=True를 명시하세요.",
        )
    if delete_all:
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        WEEK03_TOOL_CALL_PROMPT,
        """
너는 Week 3 agent로 자연어 요청을 구조화해서 SQLite에 저장하고, 저장된 일정/할 일/알림을 조회·수정·삭제해.
이번 주차에서는 반드시 여기까지만 다뤄.
""",
    ]
