...
SQLITE_MEMORY_PROMPT = """너는 Week 3부터 SQLite에 영구 저장된 데이터를 다룰 수 있어.
새 대화를 시작하거나 서버가 재시작되어도 SQLite에 저장된 데이터는 그대로 남아있으니,
이전에 저장한 일정/할 일/알림도 항상 존재한다고 가정하고 답해."""

WEEK03_TOOL_CALL_PROMPT = "\n".join([
    "일정, 할 일, 알림을 새로 생성하는 요청이 오면, 1~2주차의 생성 관련 tool은 메모리에만 저장되고 SQLite에는 남지 않으니 절대 호출하지 마. 대신 먼저 extract_schedule_request(query)로 사용자 요청을 구조화한 뒤, 그 결과의 structured_request 필드(kind/title/date/start_time/end_time/members/priority/reason/original_text)를 save_structured_request tool의 인자로 그대로 전달해서 저장해.",
    "save_structured_request는 kind에 따라 structured_requests 테이블에는 항상 원본을 저장하고, personal_schedule/group_schedule이면 schedules, todo면 todos, reminder면 reminders 테이블에도 함께 저장해. unknown은 structured_requests에만 저장돼.",
    "사용자가 '지금까지 요청한 것 전체', '전체 기록'처럼 종류에 상관없이 포괄적으로 조회하려 하면 list_saved_requests(여러 건) 또는 get_saved_request(request_id로 단건)를 사용해. 이 tool들은 structured_requests 테이블 전체를 조회 대상으로 한다.",
    "사용자가 '내 일정 보여줘'처럼 일정만 조회하려 하면 personal_list_saved_schedules를 사용해. 이 tool은 schedules 테이블만 조회하므로 todo나 reminder, unknown으로 분류된 요청은 나오지 않는다.",
    "일정 수정이나 삭제 요청이 오면, 현재는 SQLite 기반으로 이 기능이 구현되어 있지 않으니 tool을 호출하지 말고, 아직 지원하지 않는 기능이라고 사용자에게 안내해.",
])
...
def week03_tools() -> list[Any]:
    ...
    # NOTE: Week 3의 personal_create_schedule(SQLite 이중 기록 버전)이 아직 미구현(TODO) 상태라서,
    # Week 1의 personal_create_schedule(메모리 전용)과 함께 목록에서 완전히 제외한다.
    # Week 3의 personal_create_schedule을 완성하면, Week 1의 personal_create_schedule을 Week 3 버전으로 대체하도록 수정한다.
    
    base_tools = [item for item in week01_tools() if _tool_name(item) != "personal_create_schedule"]
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "Week 2에서 자연어를 kind/title/date 등의 필드로 구조화했던 것을, Week 3에서는 save_structured_request tool의 인자로 그대로 사용해서 SQLite에 저장해.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"오늘은 {current_app_date_iso()}이야. 상대 날짜는 이 날짜 기준으로 계산해.",
        "Week 3에서는 저장과 조회까지만 다루고, 일정 수정/삭제, RAG, 외부 멤버 일정 조율은 하지 않아.",
    ]
