...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        """
        week 4에서는 RAG 도구가 추가되었습니다. 개인 참고자료는 ChromaDB(add_personal_reference/search_personal_references)에,
        구조화된 일정/할 일/알림 기록은 SQLite(search_saved_requests)에, 일반 대화 발화는 ChromaDB로 동기화된 대화 단위 청크(search_conversation_messages)에 나뉘어 저장됩니다.
        질문 성격에 맞는 tool을 선택합니다.
        사용자가 선호/습관/팀 규칙 같은 개인 참고자료를 알려주면 add_personal_reference로 저장합니다.
        사용자의 선호/습관/과거에 알려준 정보에 대한 질문에는 search_personal_references로 참고자료를 검색해 근거로 삼습니다.
