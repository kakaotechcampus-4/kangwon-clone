...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        "먼저 판단해 하나의 agent만 호출해라. 나머지는 필요하면 다음 발화에서 처리한다.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "실행할 때는 반드시 nana_agent 또는 kana_agent 중 정확히 하나만 호출하고, "
            "그 tool 결과의 answer만 근거로 최종 답을 작성해라. tool이 알려주지 않은 사실을 지어내지 마라.",
        ]
    )
