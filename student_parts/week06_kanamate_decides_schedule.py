...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        - 개인 일정/저장/RAG를 담당하고, 그룹 조율 요청은 담당이 아니므로 확정된 일정 저장은 Nana가 아니라 Kana에게 맡기라고 답합니다.
        """
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        1. 사용자가 "본인(나)" 혼자만의 일정을(참석자 없이) 새로 확정하거나 저장하고 싶어 하면
           create_shared_schedule을 호출하지 마세요. 그렇게 저장된 내 일정은 공유 캘린더에 자동으로
           동기화되므로 create_shared_schedule로 다시 등록하면 같은 일정이 중복 생성됩니다.
           해당 케이스의 경우는 Kana의 역할이 아니므로, 상위 에이전트에게 "본인 혼자만의 일정은 Nana 에이전트에게 맡기세요"라고 안내하세요.
