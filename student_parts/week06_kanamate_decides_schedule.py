...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "사용자의 요청에 날짜나 요일에 대한 언급이 없다면 date_from과 date_to를 비워둬야 해.",
        "사용자의 요청에 따로 개인적인 일정인지 단체의 일정인지에 대한 언급이 없다면 kind를 비워둬야 해.",
        "사용자가 '내 일정 보여줘' 라고 한다면 개인 일정과 단체 일정을 모두 알려줘야 해.",
    ]
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    ...
    f"최적 날짜는 {current_app_date_iso()} 이후의 날짜여야 해. "
    "busy_rows에 있는 날짜는 일정이 있는 날이고 busy_rows에 있지 않은 날짜들은 모두 일정이 비어있는 날이야. "
    "날짜는 겹치더라도 겹치지 않는 시간대가 있다면 일정을 잡을 수 있어. "
    "같은 날에 11:00부터 12:00까지 일정이 있더라도 12:00부터 13:00은 일정 후보가 될 수 있어. "
    "계산을 마친 후 계산된 날짜들에 대한 정보들을 candidate_slots에 채워 넣어야 해. "
    "candidate_slots를 비운 상태로 호출하면 반환값도 빈 상태로 나와. "
    "candidate_slots는 꼭 후보를 1개 이상 채워서 호출 해. "
    ...
    "요청한 일정의 길이는 duration_minutes이고 start_time에서 duration_minutes을 더한 값이 end_time이야. "
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    ...
    "selected_index나 selected_slot 없이 final_slot만 채워서는 안돼. "
    "내가 일정의 확정을 요청했을 때에만 일정 확정을 해주고 조회만 요청했다면 final_slot은 null로 두고 needs_agent_selection은 true로 해둬야 해. "
...
#   호환용 그룹 일정 제안 tool(구현 완료)의 입력 스키마입니다.
...
# [공통] AgentQueryInput
#   supervisor가 하위 agent에 query를 넘기는 wrapper tool(메인과제)의 입력 스키마입니다.
...
def decide_final_slot(
    ...
    return json_payload(decide_final_slot_payload(
        candidate_slots= candidate_slots,
...
# [공통] agent_tool_names(agent_name)
#   Kana 하위 agent와 supervisor가 볼 수 있는 tool 목록을 역할별로 조립하고 이름 목록을 제공합니다.
...
# [공통] build_langchain_supervisor_agent()
#   supervisor agent를 한 번만 만들고 재사용합니다.
#
#   구현 위치와 사용할 코드
#     - nana_agent()/kana_agent()/build_langchain_supervisor_agent()는 create_agent(...)로 각각 필요한 agent를 만들고 재사용합니다.
