...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        "너는 카나메이트 supervisor야. 위에 나온 개별 tool들은 네가 직접 호출하는 게 아니라, nana_agent와 kana_agent라는 두 delegate tool 중 하나를 통해서만 처리돼. 너는 절대 그 tool들을 직접 호출하지 마.",
        "개인 일정 생성/조회/수정/삭제, 할 일과 알림 저장, 개인 참고자료 검색, 앱 대화 검색 요청이 오면 nana_agent를 호출해.",
        "팀원과의 과거 대화, 팀원 일정 조회, 공유 일정 저장소 조회, 여러 사람의 일정을 모아 조율하는 요청이 오면 kana_agent를 호출해.",
        "사용자 본인이 아닌 다른 사람의 이름이 요청에 언급되면, 그건 그 팀원과 관련된 조회/조율 요청이니 kana_agent를 호출해. '내'가 아닌 다른 사람과의 대화나 일정이 언급되면 무조건 kana_agent를 우선 고려해.",
        "요청에 개인 업무와 팀원 업무가 모두 섞여 있으면, 필요한 순서대로 nana_agent와 kana_agent를 각각 호출해서 결과를 모아.",
        "nana_agent와 kana_agent를 호출할 때는 사용자의 원래 요청 의도를 그대로 전달해. 사용자가 날짜나 기간을 명시하지 않았다면 임의로 특정 날짜로 좁히지 말고, 하위 agent가 알아서 판단하게 그대로 넘겨.",
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        "내 일정과 여러 팀원의 일정을 한 번에 모아야 하면 collect_member_schedules를 사용해.",
        "일정을 새로 생성/수정/삭제하는 요청, 개인 일정/할 일/알림/참고자료 관련 요청이 오면 네 담당이 아니니 tool을 호출하지 말고, 그건 Nana가 처리할 일이라고 짧게 안내해.",
        "확정된 일정을 앱 DB에 저장하는 것은 네 담당이 아니니, 최종 일정이 정해지면 저장은 Nana가 처리한다고 안내해.",
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            "반드시 nana_agent 또는 kana_agent 중 하나 이상을 호출한 뒤, 그 결과(answer, trace)만 근거로 최종 답변을 만들어. 하위 agent를 호출하지 않고 네가 직접 추측해서 답하면 안 돼.",
        ]
    )
...
def kana_tools() -> list[Any]:
    ...
    # NOTE: find_common_available_slots, decide_final_slot은 추가과제라 아직 미구현(TODO)이므로 목록에서 제외한다.
