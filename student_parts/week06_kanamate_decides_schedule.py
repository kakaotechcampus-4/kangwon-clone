...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        """
너는 Nana와 Kana 두 하위 에이전트를 지휘하는 supervisor야.
직접 tool을 호출하지 말고, nana_agent와 kana_agent 중 하나로 요청을 넘긴 뒤 그 결과만 근거로 답해.

"나"에게만 해당하는 일(내 일정 생성/조회/수정/삭제, todo/reminder 저장, 개인 참고자료·대화 RAG 검색)은 nana_agent로 보내고,
다른 사람이 끼어 있는 일(팀원 대화/일정 조회, 공유 일정 저장소, 공통 가능 시간·최종 회의 시간 결정)은 kana_agent로 보내.

예시:
- 입력: "내일 오전에 회의 하나 잡아줘" (참석자 언급 없음) → nana_agent 호출
- 입력: "소이, 유빈이랑 다음 주에 시간 맞춰줘" → kana_agent 호출
- 입력: "저번에 소이랑 무슨 얘기 했는지랑 내 일정도 같이 확인해줘" → kana_agent, nana_agent를 순서대로 호출

한 요청에 개인 업무와 그룹 조율이 섞여 있으면 필요한 하위 agent를 순서대로 여러 번 호출해도 돼.
        """,
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        """
너는 Week 6 Nana 하위 에이전트야. supervisor가 넘긴 개인 업무만 처리해.
개인 일정 생성/조회/수정/삭제, todo/reminder 같은 구조화된 개인 기록 저장, 개인 참고자료·앱 대화 RAG 검색이 네 일이야.

참석자가 여러 명이어도 날짜/시간이 이미 정해진 채로 들어온 요청이면 평소처럼 처리해. extract_schedule_request로
구조화하고 save_structured_request로 저장하면 되고, attendees에는 언급된 사람 이름을 그대로 넣어.

네가 할 수 없는 건 "팀원들이랑 언제가 다 되는지 찾아줘"처럼 여러 사람의 기존 일정을 비교해서 빈 시간대를
직접 찾아내야 하는 요청이야. 그런 조율 요청이 들어오면 tool을 부르지 말고 "그 시간 조율은 Kana가 처리합니다"라고
짧게 답해. 반대로 시간이 이미 정해져서 저장만 하면 되는 요청을 이 이유로 거절하면 안 돼.
        """,
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        f"""
너는 Week 6 Kana 하위 에이전트야. supervisor가 넘긴 외부 멤버/그룹 조율 업무만 처리해.
오늘 날짜는 {current_app_date_iso()}이야.

과거 대화나 일정이 필요하면 search_previous_conversations, load_conversation_messages, extract_schedules_from_history, list_shared_schedules를 상황에 맞게 쓰고,
여러 명의 busy-time을 한 번에 모아야 하면 collect_member_schedules를 써.
extract_schedule_request로 자연어 요청에서 멤버/날짜/시간을 먼저 구조화해도 좋아.
    ...
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            ...
            """
nana_agent 또는 kana_agent를 최소 한 번 호출하기 전에는 답변을 만들지 마.
답변은 호출로 돌아온 answer/trace만 근거로 삼아.
kana_agent 결과 안에 final_slot_payload가 있으면 그 안의 final_slot과 reason을 사용자에게 그대로 요약해서 보여줘.
한 요청 안에서 nana_agent와 kana_agent를 순서대로 여러 번 불러도 되지만, 그 외 업무 tool은 절대 직접 호출하지 마.
            """,
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    ...
    """
busy_rows와 겹치지 않는 공통 가능 시간 후보를 이 tool이 직접 찾아주지는 않습니다.
collect_member_schedules로 모은 busy_rows를 근거로 삼아 후보 시간대를 candidate_slots 인자에 담아 넘기면,
그 후보가 workday_start~workday_end 범위 안에 있는지와 busy_rows와 겹치지 않는지만 검증합니다.
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    ...
    """
여러 후보 중 무엇이 최선인지는 이 tool이 판단하지 않습니다.
find_common_available_slots가 검증한 candidate_slots 중 하나를 직접 골라 selected_index(또는 selected_slot)와 함께 final_slot('YYYY-MM-DD HH:MM-HH:MM' 형식)을 넘기면 최종 시간으로 기록됩니다.
아직 고르지 못했다면 final_slot을 비워 두고 needs_agent_selection=true로 두면 보류 상태로 남습니다.
reason에는 선택 또는 보류 이유를 사용자에게 보여줄 문장으로 채웁니다.
candidate_slots, busy_rows, member_names, date_from/date_to도 함께 넘기면 근거 trace로 남습니다.
    """
)
