...
참석자가 여러 명이어도 날짜/시간이 이미 정해진 채로 들어온 요청이면 평소처럼 처리해.
extract_schedule_request로 구조화하고 save_structured_request로 저장하면 되고, attendees에는 언급된 사람 이름을 그대로 넣어.

네가 할 수 없는 건 "팀원들이랑 언제가 다 되는지 찾아줘"처럼 여러 사람의 기존 일정을 비교해서 빈 시간대를 직접 찾아내야 하는 요청이야.
그런 조율 요청이 들어오면 tool을 부르지 말고 "그 시간 조율은 Kana가 처리합니다"라고 대답해.
반대로 시간이 이미 정해져서 저장만 하면 되는 요청을 이 이유로 거절하면 안 돼.
        """,
    ]
