...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    """
    수집된 busy_rows를 근거로, 당신이 직접 고른 공통 가능 시간 후보가 유효한지 검증하는 tool입니다.
    이 tool은 후보를 스스로 계산하지 않으므로, 앞선 tool 결과의 busy_rows를 그대로 복사해 넘기고 candidate_slots에 후보를 직접 채워 넘겨야 합니다.
    candidate_slots 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, reason을 포함해야 하며, 후보는 busy_rows의 어떤 row와도 겹치면 안 됩니다.
    이 결과만으로 답변을 끝내지 말고, 검증된 후보를 근거로 decide_final_slot을 이어서 호출해 최종 시간을 확정하세요.
"""
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    """검증된 후보 중에서 최종 일정 시간을 기록하는 tool입니다. 
    이 tool은 최종 시간을 스스로 고르지 않으므로, find_common_available_slots가 검증한 candidate_slots 중 
    하나를 직접 선택해 selected_index 또는 selected_slot과 final_slot('YYYY-MM-DD HH:MM-HH:MM' 형식)을 채워 넘기세요. 
    아직 확정할 근거가 부족하면 final_slot은 비워두고 needs_agent_selection을 true로 둡니다. 
    근거 추적을 위해 candidate_slots, busy_rows, member_names, date_from/date_to도 함께 넘기고, 
    reason에는 선택 또는 보류 이유를 사용자에게 설명하듯 적으세요."""
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        collected = json.loads(
            collect_member_schedules.invoke({"member_names":normalized_member_names, "date_from":normalized_date_from, "date_to":normalized_date_to}))
