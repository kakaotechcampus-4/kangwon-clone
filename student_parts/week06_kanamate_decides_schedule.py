...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "너는 Week 6 supervisor다. 사용자의 요청을 직접 처리하거나 이전 주차 도구를 직접 호출하지 말고 "
            "반드시 nana_agent 또는 kana_agent 중 하나에 위임한다."
        ),
        (
            "내 개인 일정의 생성·조회·수정·삭제, 할 일과 알림 저장, 개인 참고자료, 앱 내부 이전 대화 검색은 "
            "nana_agent에 위임한다."
        ),
        (
            "철수나 영희 같은 외부 멤버, 공유 일정, 외부 이전 대화, 여러 사람의 바쁜 시간 수집, "
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "너는 Week 6 Nana 하위 에이전트다. 내 개인 일정의 생성·조회·수정·삭제, 할 일과 알림 저장, "
            "개인 참고자료, 앱 내부 이전 대화 RAG만 담당한다."
        ),
        (
            "요청을 완료하려면 제공된 개인 도구를 실제로 호출하고 도구 결과를 확인한 뒤 답한다. "
            "외부 멤버 일정, 공유 일정, 여러 사람의 공통 시간이나 그룹 일정 조율은 담당하지 않는다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            "너는 Week 6 Kana 하위 에이전트다. "
            f"현재 날짜는 앱 시작 시 OS에서 읽은 {current_app_date_iso()}이다."
        ),
        (
            "외부 SQLite의 이전 대화 검색과 메시지 조회, 외부 멤버 일정 추출, 공유 일정 조회, "
            "내 일정과 외부 멤버의 바쁜 시간 통합, 공통 가능 시간 검증과 최종 시간 결정을 담당한다."
        ),
        (
            "외부 멤버나 그룹 조율 요청에서는 필요한 도구만 순서대로 호출한다. "
            "여러 사람의 일정이 필요하면 collect_member_schedules를 사용해 내 일정과 외부 멤버 일정을 함께 수집한다."
        ),
        (
            "공통 가능 시간을 요청받으면 collect_member_schedules 결과의 rows를 busy_rows로 사용한다. "
            "busy_rows의 kind, source, title과 관계없이 모든 행을 이미 바쁜 시간으로 간주하며, "
            "이미 잡힌 개인 일정과 그룹 일정을 빈 시간으로 추천하지 않는다."
        ),
        (
            "busy_rows를 직접 읽고 요청 날짜 범위, 업무 시간, 회의 길이를 만족하며 어느 busy row와도 겹치지 않는 "
            "candidate_slots를 직접 고른다. 각 후보는 date, start_time, end_time, duration_minutes, reason을 포함한다."
        ),
        (
            "고른 후보를 busy_rows와 함께 find_common_available_slots에 전달해 검증하고, "
            "검증된 후보 중 하나를 직접 선택해 decide_final_slot을 이어서 호출한다. "
            "최종 시간 형식은 'YYYY-MM-DD HH:MM-HH:MM'으로 작성한다."
        ),
        (
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "여러 사람의 공통 가능 시간 후보를 검증할 때 사용한다. 이 Python tool은 후보 시간을 자동으로 계산하거나 "
    "선택하지 않는다. 먼저 collect_member_schedules 등에서 받은 rows를 busy_rows에 그대로 복사하고, "
    "Kana agent가 busy_rows를 직접 읽어 어느 일정과도 겹치지 않는 candidate_slots를 직접 만들어 전달해야 한다. "
    "busy_rows는 kind나 source와 관계없이 모두 바쁜 시간이며 이미 잡힌 그룹 일정도 반드시 제외한다. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason을 포함해야 한다. member_names, date_from, date_to, duration_minutes, "
    "workday_start, workday_end, limit도 요청에 맞게 전달한다. 검증 결과만으로 답변을 끝내지 말고 "
    "유효한 후보 중 하나를 직접 고른 뒤 decide_final_slot을 이어서 호출한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "검증된 공통 가능 시간 후보 중 최종 시간을 기록할 때 사용한다. 이 Python tool은 최종 시간을 자동으로 선택하지 "
    "않으므로 Kana agent가 candidate_slots를 읽고 selected_index 또는 selected_slot을 직접 선택해야 한다. "
    "선택한 시간과 일치하는 final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 전달하고 "
    "needs_agent_selection은 false, reason은 선택 이유로 작성한다. 아직 선택할 수 없거나 유효한 후보가 없으면 "
    "final_slot은 null, needs_agent_selection은 true로 둔다. 근거가 trace에 남도록 candidate_slots, busy_rows, "
    "member_names, date_from, date_to, duration_minutes를 함께 전달한다."
)
...
def _dict_rows(payload: Any) -> list[dict[str, Any]]:
...
def _slot_dicts(values: list[Any] | None) -> list[dict[str, Any]]:
    return [
...
def find_common_available_slots_dict(
    ...
    return find_common_available_slots_payload(
        ...
        limit=limit,
        busy_rows=normalized_busy_rows,
        candidate_slots=_slot_dicts(candidate_slots),
...
def decide_final_slot(
    ...
    payload = decide_final_slot_payload(
        ...
        member_names=normalize_external_member_names(member_names or []),
        date_from=normalize_date_bound(date_from) if date_from else None,
        date_to=normalize_date_bound(date_to) if date_to else None,
        duration_minutes=duration_minutes,
        reason=reason,
        busy_rows=[row for row in busy_rows or [] if isinstance(row, dict)],
    )
    return json.dumps(payload, ensure_ascii=False)
...
def kana_agent(query: str) -> str:
    ...
    for event in events:
        ...
        if content.get("final_slot_payload"):
            final_slot_payload = content["final_slot_payload"]
        elif event.get("tool_name") == "decide_final_slot" or "final_slot" in content:
            final_slot_payload = content
        if content.get("final_decision_payload"):
            final_decision_payload = content["final_decision_payload"]
