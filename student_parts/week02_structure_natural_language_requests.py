...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="구조화된 요청 목록. 하나뿐이어도 리스트로.")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜(오늘/내일 등) 해석 기준이 되는 오늘 날짜.")
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError("StructuredRequest 형식이 아닙니다.")
