...
class StructuredRequest(BaseModel):
    ...
    title: str | None = Field(
        default=None, description="일정이나 할 일의 제목. 모르면 None"
    )
    date: str | None = Field(
        default=None,
        description="일정 날짜를 YYYY-MM-DD 형식으로 채워야 해. '내일','다음 주 금요일' 같은 상대 표현은 base_date를 기준으로 계산하고, 날짜를 알 수 없으면 None으로 둬.",
    )
    ...
    priority: str | None = Field(
        default=None, description="할 일 우선순위. 언급 없으면 None"
    )
    reason: str | None = Field(
        default=None, description="이렇게 판단한 근거. 필요 없으면 None"
    )
    original_text: str = Field(default="", description="사용자가 입력한 원문을 그대로")


class StructuredRequestBatch(BaseModel):
    """여러 자연어 의도를 StructuredRequest 목록으로 나누는 메인과제 스키마입니다."""

    # 한 문장에 "내일 오후 7시에 회의 잡아주고 오늘 오전 10시에 카테캠 과제 좀 하라고 리마인더 시켜줘" 같은 복합 구조가 나타나면
    # 여러 개가 섞일 수 있기 때문에 단일 StructuredRequest가 아니라 list로 각각 분리해서 담기로 했다.
    requests: list[StructuredRequest] = Field(
        default_factory=list,
        description="요청이 하나뿐이어도 StructuredRequest 하나를 담은 list",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="상대 날짜 해석 기준일. '내일', '다음 주 금요일' 같은 표현을 계산할 때 기준이 되는 날짜를 YYYY-MM-DD 형식으로 담습니다.",
    )


def _coerce_structured_request(value: Any) -> StructuredRequest:
    """LangChain structured output 결과를 StructuredRequest로 정규화합니다."""
    # LLM 결과는 dict로 오거나, structured output으로 오거나 둘 중 하나이다.
    # - StructuredRequest 객체 그대로 -> 그대로 반환
    # - dict (ex: {"kind": "todo", "title": "회의 준비"}) -> 검증해서 객체로 변환
    # 그 외 예상한 형태가 아니면 에러를 발생시켜서 LLM 응답을 그냥 넘어가지 않게 설정
    ...
    raise RuntimeError(
        "LLM structured output이 예상한 형태가 아닙니다. "
        f"StructuredRequest 또는 dict여야 하는데 {type(value)}입니다."
    )


def extract_structured_request(text: str) -> StructuredRequest:
    """Week 3 이상에서 agent를 새로 띄우지 않고 자연어를 StructuredRequest로 바꿉니다."""
    # method="function_calling"을 써서 LLM이 StructuredRequest 스키마를 이해하고, JSON으로 반환하도록 강제한다.
    # 이 과정에서 프록시 모델이 content에 임의 텍스트가 붙는 것을 방지하였음
    ...
    return _coerce_structured_request(output_LLM)
...
def extract_schedule_request(query: str) -> str:
    ...
    structured_request = extract_structured_request(query)  # 구조화 하는 부분
    result = {  # dict로 만들어서 JSON으로 반환
        "ok": True,
        "tool_name": "extract_schedule_request",
        "base_date": current_app_date_iso(),
        "structured_request": structured_request.model_dump(),  # 객체 -> dict로 변환
    }
...
def week02_system_prompt() -> str:
    ...
    # 프롬프트 내용은 전부 week02_prompt_parts()에 모아두고,
    # 이 함수는 조각들을 하나의 문자열로 합치는 역할만 하도록 나눴다.
    # 그래야 프롬프트를 수정할 때 parts 한 곳만 보면 된다.
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        "요청이 하나뿐이어도 requests 리스트에 StructuredRequest 하나를 담아야 해.",
        f"너는 자연어 요청을 StructuredRequestBatch로 구조화하는 agent야. 오늘은 {current_app_date_iso()}이고 상대 날짜는 이 기준으로 계산해야 해.",
        "사용자 말에서 kind/title/date/start_time/members 등을 뽑아 채워야 해. 확실하지 않은 값은 지어내지 말고 None이나 빈 list로 둬야 해.",
        "week01 tool 결과(created_schedule JSON)를 받으면 tool을 다시 호출하지 말고 그 payload의 값을 그대로 읽어서 구조화해야 해.",
        "week2에선 SQLite 저장·RAG·외부 멤버 조율은 하지 않아야 해. ",
    ]
...
def build_week02_agent() -> object:
    ...
    if _WEEK02_AGENT is None:
        _WEEK02_AGENT = create_agent(
            model=chat_model(),
            tools=week02_tools(),
            # 없으면 자유롭게 답하지만, 넣으면 답을 반드시 StructuredRequestBatch 형식으로만 내도록 강제된다.
            # 스키마를 그대로 줬더니 native JSON 모드가 자동 선택됐는데 프록시 모델에서
            # 응답 뒤에 여분 데이터가 붙어 파싱 에러가 났습니다.
            # 트러블 슈팅을 AI에게 부탁해보았는데 ToolStrategy(StructuredRequestBatch)로 명시적으로 지정하라고 해서 그렇게 했습니다.
            # 하지만 이 부분에 대한 이해가 되지 않았습니다...
