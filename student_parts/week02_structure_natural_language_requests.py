...
class StructuredRequest(BaseModel):
    ...
    original_text: str = Field(default="", description="원문 그대로 저장.")


class StructuredRequestBatch(BaseModel):
    """여러 자연어 의도를 StructuredRequest 목록으로 나누는 메인과제 스키마입니다."""

    requests: list[StructuredRequest] = Field(default_factory=list, description="생성된 StructuredRequest를 리스트로 저장해 관리.")
