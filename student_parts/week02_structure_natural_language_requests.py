...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field (
        description="요청 종류를 나타내는 필드. personal_schedule, group_schedule, todo, reminder, unknown 중 하나로 설정."
    )

    # TODO: title/date/start_time/end_time 필드를 str | None 타입으로 선언하고 기본값은 None으로 두세요.
    title: str | None = Field(default=None, description="요청 제목을 나타내는 필드. 문자열 또는 None으로 설정.")
    date: str | None = Field(default=None, description="요청 날짜를 나타내는 필드. YYYY-MM-DD 형식의 문자열 또는 None으로 설정.")
    start_time: str | None = Field(default=None, description="요청 시작 시간을 나타내는 필드. HH:MM 형식의 문자열 또는 None으로 설정.")
    end_time: str | None = Field(default=None, description="요청 종료 시간을 나타내는 필드. HH:MM 형식의 문자열 또는 None으로 설정.")

    # TODO: members 필드를 list[str] 타입으로 선언하고 default_factory=list를 사용하세요.
    members: list[str] = Field(default_factory=list, description="요청 관련 멤버를 나타내는 필드. 문자열 리스트로 설정.")

    # TODO: priority/reason 필드를 str | None 타입으로 선언하고 기본값은 None으로 두세요.
    priority: str | None = Field(default=None, description="요청 우선순위를 나타내는 필드. 문자열 또는 None으로 설정.")
    reason: str | None = Field(default=None, description="요청 근거를 나타내는 필드. 문자열 또는 None으로 설정.")

    # TODO: original_text 필드를 str 타입으로 선언하고 기본값은 ""로 두세요.
    original_text: str = Field(default="", description="원문을 나타내는 필드. 문자열로 설정.")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description=(
            "여러 StructuredRequest를 담는 필드."
            "StructuredRequest 객체의 리스트로 설정."
            "요청이 하나뿐이어도 반드시 list에 StructuredRequest 하나를 담아 반환."
            "절대 StructuredRequest를 단독 객체로 반환하지 않아야 함."
        )
    )

    # TODO: base_date 필드를 str 타입으로 선언하고 default_factory=current_app_date_iso를 사용하세요.
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜 해석 기준일을 나타내는 필드. 현재 앱 날짜를 ISO 형식의 문자열로 설정.")
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError(f"예상한 형태의 Structured output 형태가 아닙니다: {type(value)}")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt([
        *week02_prompt_parts(),
        ...
        (
            "요청이 하나뿐이어도 requests 리스트에 StructuredRequest 하나를 담아 반환하고, "
            "절대 단독 객체로 반환하지 않는다."
        ),

        # TODO: personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 필드를 채우도록 지시하세요.
        (
            "개인 일정 생성 요청이면 personal_create_schedule을 호출한 뒤 결과 JSON의 "
            "created_schedule을 읽어 StructuredRequest 필드를 채운다."
        ),
    ])
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        (
            "지금부터 너는 Week 2 자연어 요청 구조화 agent 역할을 수행한다. "
            f"오늘은 {today_iso}({day_name})이며, 상대 날짜 해석 기준은 오늘이다. "
        ...
        (
            "Week 1 tool(personal_create_schedule, personal_list_schedules, personal_delete_schedule)의 결과 JSON을 "
            "이미 받았다면 tool을 다시 호출하지 않고, 그 payload를 읽어 구조화 필드를 채운 뒤 바로 structured_response로 답한다."
        ),

        # TODO: Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율을 하지 않는다고 명시하세요.
        (
            "Week 2 agent는 SQLite 저장, RAG 검색, 외부 멤버 일정 조율을 하지 않는다. 구조화 결과만 만든다."
        ),
    ]
