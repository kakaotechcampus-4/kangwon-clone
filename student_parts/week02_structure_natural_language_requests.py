...
class StructuredRequest(BaseModel):
    ...
    kind : RequestKind = Field(description="요청의 종류")
    ...
    priority : str | None = Field(default=None, description= "요청의 우선 순위를 정한다.")
    reason : str | None = Field(default=None, description="우선 순위의 판단 이유를 설명한다.")
    original_text : str = Field(default="", description="원본 문장")
    # 얘는 왜 str | None이 아니라 str일까? 궁금증 발생
    # -> 항상 사용자의 입력을 받은 뒤 만들어지므로 반드시 존재하는 값임. 원문 보존이 목적


class StructuredRequestBatch(BaseModel):
    """여러 자연어 의도를 StructuredRequest 목록으로 나누는 메인과제 스키마입니다."""
    
    requests : list[StructuredRequest] = Field(default_factory=list, description="구조화된 요청")
    base_date : str = Field(default_factory=current_app_date_iso, description="상대 날짜를 계산하기 위한 기준 날짜")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        ...
        [
            (
                "최종 응답은 무조건 StructuredRequestBatch 형태로 반환한다."
                "요청이 하나뿐이어도 requests 리스트 안에 StructuredRequest 하나를 넣는다."
            ),
            (
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            f"너는 사용자의 한국어 자연어 요청이나 Week 1 tool이 만든 JSON payload를 "
            f"StructuredRequestBatch 형태로 구조화하는 AI이다. "
            f"오늘 날짜는 {current_app_date_iso()}이며, "
            "상대 날짜(오늘, 내일, 다음 주 등)는 이 날짜를 기준으로 "
            "YYYY-MM-DD 형식으로 해석한다."
        ),
        (
            "사용자의 요청을 StructuredRequest의 "
            "kind, title, date, start_time, end_time, "
            "members, priority, reason, original_text 필드로 구조화해라. "
        ),
        (
            "Week 1 tool JSON을 입력으로 받은 경우 "
            "tool을 다시 호출하지 않고 payload를 읽어 "
            "StructuredRequestBatch를 생성하라."
        ),
        (
            "Week 2에서는 SQLite 저장, RAG 검색, "
            "외부 멤버 일정 조율은 수행하지 않는다. "
            "구조화된 결과만 반환한다."
        )
    ]
