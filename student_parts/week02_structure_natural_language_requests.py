...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description="요청 종류를 나타냅니다. personal_schedule(개인 일정)," 
        "group_schedule(그룹 일정), todo(할 일), reminder(상기시키기), unknown(분류 불가) 중 하나여야만 합니다."
    )

    title: str | None = Field(
        default=None,
        description="일정/할 일/상기시키기의 제목입니다. 명확하지 않다면 None으로 둡시다."
    )

    date: str | None = Field(
        default=None,
        description="YYYY-MM-DD 형식의 일정/할 일/상기시키기 날짜입니다. 명확하지 않다면 None으로 둡시다."
    )

    start_time: str | None = Field(
        default=None,
        description="HH:MM 형식의 일정/할 일/상기시키기 시작 시각입니다. 명확하지 않다면 None으로 둡시다."
    )

    end_time: str | None = Field(
        default=None,
        description="HH:MM 형식의 일정/할 일/상기시키기 종료 시각입니다. 명확하지 않다면 None으로 둡시다."
    )
    ...
    priority: str | None = Field(
        default=None,
        description="할 일 우선순위나 중요도를 나타냅니다. 명확하지 않다면 None으로 둡시다."
    )

    reason: str | None = Field(
        default=None,
        description="이 요청을 해당하는 kind와 필드들의 값으로 분류한 이유를 간단히 설명합니다."
    )


    original_text: str = Field(
        default="",
        description="사용자가 입력한 원문 요청입니다."
    )
...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    raise RuntimeError(f"올바르지 않은 값입니다: {value}")
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            ...
            (
                "만약 들어온 요청이 하나뿐이어도 StructuredRequestBatch의 requests 목록에는 StructuredRequest 객체"
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            "당신은 이제 Week 2 요청 구조화 agent입니다. "
            "사용자가 입력한 자연어 요청을 StructuredRequest 필드(kind/title/date/start_time/end_time/members/priority/reason/original_text)로 구조화합니다. "
            f"기준이 되는 현재 날짜는 {current_app_date_iso()}입니다. "
        ),
        (
            "kind 필드는 personal_schedule, group_schedule, todo, reminder, unknown 중 하나로 분류합니다. "
            "'내일', '다음 주 목요일'과 같은 상세 날짜는 기준 날짜를 기준으로 YYYY-MM-DD 형식으로 변환합니다. "
        ),
        (
            "만약 Week 1 tool JSON을 받은 경우, 다시 tool을 호출하지 않고 payload를 읽어 structured_response로 만듭니다. "
            "값을 임의로 만들어내지 않습니다. "
        ),
        (
