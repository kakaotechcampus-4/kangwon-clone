...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="사용자의 요청 종류. RequestKind Literal에 들어 있는 값만 허용")
    title: str|None = Field(default=None, description="일정의 제목")
    date: str|None = Field(default=None, description="일정 날짜. 확실할 때만 YYYY-MM-DD 형식")
    start_time: str|None = Field(default=None, description="일정 시작 시간. 확실할 때만 HH:MM 형식")
    end_time: str|None = Field(default=None, description="일정 종료 시간. 확실할 때만 HH:MM 형식")
    members: list[str] = Field(default_factory=list, description="참석자/관련 멤버 리스트. 모르면 빈 list")
    priority: str|None = Field(default=None, description="할 일 우선 순위")
    reason: str|None = Field(default=None, description="우선순위 판단 근거")
    original_text: str = Field(default="", description="사용자의 원문 텍스트")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="여러 자연어 의도를 StructuredRequest 리스트 형태로 저장. 요청이 하나여도 리스트 형태 유지")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜 기준일. YYYY-MM-DD 형식")
...
def week02_system_prompt() -> str:
    ...
    week02_final_answer_rule ="""
        최종 답변은 반드시 StructuredRequestBatch 형식으로 출력하세요.
        요청이 하나 뿐이더라도 requests 목록 안에 StructuredRequest를 하나만 담아 반환하세요.
        personal_create_schedule tool을 호출했다면, 그 결과 JSON의 created_schedule을 읽어 StructuredRequest 필드를 채우세요.
    """
    return join_system_prompt([*week02_prompt_parts(),week02_final_answer_rule])
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"""
            Nana는 추가로 사용자의 요청이나 tool이 반환한 JSON을 StructuredRequest 필드(kind/title/date/start_time/end_time/members 등)로 구조화하는 역할도 맡습니다.
            오늘 날짜는 {current_app_date_iso()}입니다. 상대 날짜를 이 날짜 기준으로 계산합니다.
            이미 Week 1 tool을 호출해 결과 JSON을 받은 경우 다시 tool을 호출하지 않고 payload를 읽어 structured_response를 만드세요.
            이번 Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율 역할을 담당하지 않습니다.
        """
    ]
