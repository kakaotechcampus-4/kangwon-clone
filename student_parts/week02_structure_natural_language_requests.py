...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="사용자 요청 종류")
    title: str | None = Field(default=None, description="일정 제목 또는 요청 주제")
    date: str | None = Field(default=None, description="확실히 알 수 있는 일정 날짜. YYYY-MM-DD 형식")
    start_time: str | None = Field(default=None, description="확실히 알 수 있는 일정 시작 시간. HH:MM 형식")
    end_time: str | None = Field(default=None, description="확실히 알 수 있는 일정 종료 시간. HH:MM 형식")
    members: list[str] = Field(default_factory=list, description="참석자 또는 관련 멤버 목록. 모르면 빈 목록")
    priority: str | None = Field(default=None, description="할 일이나 요청의 우선순위")
    reason: str | None = Field(default=None, description="우선순위 또는 구조화 판단 근거")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="사용자 자연어 요청에서 추출한 StructuredRequest 목록. 요청이 하나여도 list 형태",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="상대 날짜 표현을 절대 날짜로 계산하기 위한 기준일. YYYY-MM-DD 형식",
    )
...
def week02_system_prompt() -> str:
    ...
    week2_rules = [
        "[Week 2 최종 답변 규칙]",
        "사용자의 요청이 하나뿐이어도 StructuredRequestBatch.requests에는 반드시 StructuredRequest 하나를 담은 list를 반환하라.",
        "개인 일정 생성 요청에서는 personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 구조화 필드를 채워라.",
    ]
