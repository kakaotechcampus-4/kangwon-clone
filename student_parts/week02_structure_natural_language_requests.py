...
def _coerce_structured_request(value: Any) -> StructuredRequest:
    ...
    if isinstance(value, StructuredRequest):
        ...
        raise RuntimeError(f"Unexpected value type: {type(value)}. Expected StructuredRequest or dict.")
...
def extract_structured_request(text: str) -> StructuredRequest:
    ...
    user_message = text
    response = structured_llm.invoke([{"role": "system", "content": system_prompt}, {"role": "user", "content": user_message}])
...
def week02_prompt_parts() -> list[str]:
    ...
    payload_prompt = "Week 1 personal_create_schedule tool 결과 JSON의 created_schedule을 읽어 StructuredRequest 필드를 채우세요."
    week02_prompt = "Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율을 하지 않습니다."
    return [
        *week01_prompt_parts(),
        now_prompt,
        field_prompt,
