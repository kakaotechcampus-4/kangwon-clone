...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    saved_ids = {
        str(row.get("schedule_id") or row.get("id") or "").strip()
        for row in sqlite_rows
        if str(row.get("schedule_id") or row.get("id") or "").strip()
    }
    ...
    for schedule in PERSONAL_SCHEDULES:
        ...
        temporary_rows.append(dict(schedule))

    return [*sqlite_rows, *temporary_rows]
...
def _collect_member_schedules(
    *,
    ...
    normalized_date_from, normalized_date_to = (
        normalize_external_schedule_date_bounds(normalized_member_names, date_from, date_to)
    )

    rows: list[dict[str, Any]] = []

    for schedule_row in personal_schedules:
        request = _structured_request_from_schedule_row(schedule_row)
        if request.date is None:
            continue
        if request.date < normalized_date_from or request.date > normalized_date_to:
            continue
    ...
    return [
        *week04_prompt_parts(),
        ...
        (
            ...
            "검색 결과에서 특정 conversation_id의 전체 메시지가 필요하면 "
            "load_conversation_messages를 사용한다."
        ),
        (
            "외부 멤버의 이전 대화에서 일정이나 바쁜 시간을 찾을 때는 "
            "extract_schedules_from_history를 사용한다. "
            "내 일정과 외부 멤버 일정을 한 번에 비교해야 할 때는 "
            "collect_member_schedules를 사용한다."
        ),
        (
            "공유 일정 저장소에 등록된 row를 확인할 때는 list_shared_schedules를 사용한다. "
            "사용자가 공유 일정 등록이나 삭제를 명확하게 요청한 경우에만 "
        ...
        (
            ...
            "외부 MCP 결과의 conversation_id, sender, content, created_at, "
            "schedule_id, source_conversation_id를 임의로 바꾸거나 만들지 않는다."
        ),
        (
            "최종 답변은 도구가 반환한 rows, messages, schedule_summary의 실제 내용만 "
            "근거로 작성한다. 여러 사람의 최종 공통 가능 시간을 계산하거나 확정하는 것은 "
            "Week 6 범위이므로 이번 주에는 바쁜 시간 조회와 정리까지만 수행한다."
        ),
    ]
