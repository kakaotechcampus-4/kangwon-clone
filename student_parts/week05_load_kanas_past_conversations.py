...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    sql_saved_schedules_id = [schedule["schedule_id"] for schedule in sql_saved_schedules]
    
    new_schedules_list = sql_saved_schedules.copy()
    
    for schedule in this_session_schedules:
        if _schedule_scope(schedule) == current_session_scope() and schedule["id"] not in sql_saved_schedules_id:
            new_schedules_list.append(schedule)
    
    
    return new_schedules_list
...
# [메인] SearchPreviousConversationsInput
#   외부 이전 대화 검색 tool의 입력 스키마입니다.
...
# [메인] LoadConversationMessagesInput
#   대화 메시지 로드 tool의 입력 스키마입니다.
...
# [메인] ExtractSchedulesFromHistoryInput
#   외부 대화에서 일정 추출 tool의 입력 스키마입니다.
...
# [추가] CreateSharedScheduleInput
#   외부 공유 일정 저장소에 row를 생성할 때 쓰는 입력 스키마입니다.
...
# [추가] DeleteSharedScheduleInput
#   외부 공유 일정 저장소에 row를 삭제할 때 쓰는 입력 스키마입니다.
...
# [메인] ListSharedSchedulesInput
#   공유 일정 저장소 row 조회 tool의 입력 스키마입니다.
...
# [메인] CollectMemberSchedulesInput
#   내 일정·외부 멤버 busy-time을 같은 rows 배열로 합치는 tool의 입력 스키마입니다.
...
def _collect_member_schedules(
    *,
    ...
    normalized_date_from, normalized_date_to = normalize_external_schedule_date_bounds(member_names= normalized_member_names,date_from= date_from, date_to= date_to)
    
    call_mcp = call_mcp_tool_sync(tool_name= "extract_schedules_from_history", args= {
        "member_names": normalized_member_names,
        "date_from": normalized_date_from,
        "date_to": normalized_date_to
    })
    
    members_schedule = json.loads(call_mcp)["rows"]
    ...
    rowrow = members_schedule + my_schedule
    
    return {
        "rows": rowrow,
        "schedule_summary": external_schedule_summary(rowrow)
    }
...
def search_previous_conversations(
    ...
    called_mcp = call_mcp_tool_sync("search_previous_conversations", {
        "query": query,
...
def load_conversation_messages(conversation_id: str) -> str:
    ...
    loaded_messages = call_external_tool_payload("load_conversation_messages", {
        "conversation_id": conversation_id
    })
    
    return json_payload(loaded_messages)
...
#   공유 일정 저장소에 row를 등록하는 MCP wrapper입니다. source_conversation_id와 schedule_id를 보존해 동기화 근거로 씁니다.
#
#   추가 과제 구현 대상 (구현하지 않으려면 week05_tools() 목록에서 해당 tool을 제거)
#     - call_mcp_tool_sync("create_shared_schedule", args)를 호출합니다.
#     - 공유 일정 저장소 row를 생성할 때 MCP tool 결과를 그대로 전달합니다.
#     - schedule_id 또는 source_conversation_id를 보존해야 나중에 수정/삭제 동기화가 가능합니다.
#
#   구현 위치와 사용할 코드
#     - Week 3+ AppSQLiteStore는 개인/그룹 일정을 저장할 때 공유 일정 저장소에 자동 동기화할 수 있습니다.
...
#   공유 일정 저장소에 row를 삭제하는 MCP wrapper입니다. source_conversation_id와 schedule_id를 보존해 동기화 근거로 씁니다.
#
#   추가 과제 구현 대상 (구현하지 않으려면 week05_tools() 목록에서 해당 tool을 제거)
#     - call_mcp_tool_sync("delete_shared_schedule", args)를 호출합니다.
#     - 공유 일정 저장소 row를 삭제할 때 MCP tool 결과를 그대로 전달합니다.
#     - schedule_id 또는 source_conversation_id를 보존해야 나중에 수정/삭제 동기화가 가능합니다.
#
#   구현 위치와 사용할 코드
#     - Week 3+ AppSQLiteStore는 개인/그룹 일정을 저장할 때 공유 일정 저장소에 자동 동기화할 수 있습니다.
...
#   구현 위치와 사용할 코드
#     - Week 3+ AppSQLiteStore는 개인/그룹 일정을 저장할 때 공유 일정 저장소에 자동 동기화할 수 있습니다.
#       list_shared_schedules wrapper(메인)는 공유 저장소 row를 직접 확인할 때 사용합니다.
...
def list_shared_schedules(
    ...
    picked_schedules = call_mcp_tool_sync("list_shared_schedules", {
        "member_names": member_names,
        "date_from": date_from,
        "date_to": date_to,
        "source_conversation_id": source_conversation_id,
