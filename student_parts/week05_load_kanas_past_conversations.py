...
def _personal_schedules_for_current_scope(
    ...
    sqlite_rows = AppSQLiteStore(CONFIG.app_db_path).list_schedules(limit=200, date_from=date_from, date_to=date_to,)
    known_ids = {row.get("schedule_id") for row in sqlite_rows if row.get("schedule_id")}
    session_id = current_session_scope()
    temp_rows = [
        schedule for schedule in PERSONAL_SCHEDULES 
        if _schedule_scope(schedule) == session_id and schedule.get("id") not in known_ids
    ]
...
def search_previous_conversations(
    ...
    return call_mcp_tool_sync(tool_name="search_previous_conversations",args={"query": query, "member_names": member_names, "limit": limit})


@tool(args_schema=LoadConversationMessagesInput)
def load_conversation_messages(conversation_id: str) -> str:
    """외부 SQLite 데이터베이스에서 특정 이전 대화의 모든 메시지를 불러옵니다."""
    payload = call_external_tool_payload(tool_name="load_conversation_messages",args={"conversation_id":conversation_id})
...
def create_shared_schedule(
    ...
    return call_mcp_tool_sync(tool_name="create_shared_schedule", args=args)
...
def list_shared_schedules(
    ...
    return call_mcp_tool_sync(tool_name="list_shared_schedules", args=args)
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        """
        week 5에서는 외부 멤버(카나 등)의 이전 대화와 공유 일정을 다루는 도구가 추가되었습니다.
        내 개인 저장/RAG 관련 질문은 이전 주차 도구(search_personal_references, search_saved_requests, search_conversation_messages)로 처리하고,
        외부 멤버의 과거 대화나 일정은 이번 주차의 MCP 도구로 처리합니다.
        외부 멤버가 언급한 이전 대화 내용을 찾을 때는 search_previous_conversations로 먼저 검색하고,
        검색 결과 중 특정 대화의 전체 메시지가 필요하면 load_conversation_messages로 불러옵니다.
        외부 멤버의 특정 기간 일정이나 바쁜 시간을 물어보면 extract_schedules_from_history를 사용합니다.
        공유 일정 저장소에 등록된 일정 자체를 조회할 때는 list_shared_schedules를 사용합니다.
        나와 외부 멤버들의 일정을 함께 비교하거나 회의 시간을 조율해야 하면 collect_member_schedules로 rows를 모읍니다.
"""
    ]
