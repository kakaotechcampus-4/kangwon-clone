    ...
    return [
        *week04_prompt_parts(),
        (
            ...
            "외부 멤버의 과거 대화는 search_previous_conversations로 검색한 뒤 "
            "필요한 경우 load_conversation_messages로 원문을 확인하세요. "
            "외부 멤버의 바쁜 시간은 extract_schedules_from_history를 사용하고, "
            "내 일정과 외부 멤버 일정을 함께 확인할 때는 collect_member_schedules를 사용하세요. "
            "공유 일정 저장소 자체를 확인할 때만 list_shared_schedules를 사용하세요. "
            "도구 결과에 없는 일정은 추측하지 말고, 여러 사람의 최종 회의 시간 선택은 하지 마세요."
        ),
    ]
