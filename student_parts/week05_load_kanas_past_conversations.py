...
def _personal_schedules_for_current_scope() -> list[dict[str, Any]]:
    ...
    saved = AppSQLiteStore(CONFIG.app_db_path).list_schedules(limit=200, kind="personal_schedule")
    saved_ids = {str(row.get("schedule_id")) for row in saved}
...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        "Week 5부터 Nana는 '나 자신의 것'과 '다른 사람(외부 멤버)의 것'을 구분해 처리한다. "
        "내 일정·참고자료·과거 대화는 이전 주차 tool로 다루고, 다른 사람의 과거 대화나 일정은 외부 MCP wrapper tool로 다룬다.",
        "다른 사람(팀원·멤버)의 정보가 필요한 질문이면 추측하지 말고 아래 외부 tool을 호출한다. "
        "(1) 외부 멤버의 과거 대화를 찾으면 → search_previous_conversations, "
        "(2) 검색으로 찾은 특정 대화의 전체 메시지를 보려면 → load_conversation_messages, "
        "(3) 외부 멤버의 일정·바쁜 시간을 뽑으려면 → extract_schedules_from_history, "
        "(4) 공유 일정 저장소 자체를 확인하려면 → list_shared_schedules.",
        "'나와 다른 사람들의 일정을 함께 모아달라', '여러 명이 언제 시간 되는지 봐달라'처럼 "
        "내 일정과 외부 멤버 일정을 한 번에 모아야 하는 질문에는 collect_member_schedules를 사용한다. "
