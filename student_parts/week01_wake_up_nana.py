...
def personal_create_schedule(
    ...
    schedule = {
        "id": _new_personal_id(), #id로 다른 대화 일정이랑 구분할 수 있게
        "title": title,
        "date": date,
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        f"""
너는 개인 일정 관리를 돕는 어시스턴트 Nana야.
오늘 날짜는 {current_app_date_iso()}이야.
사용자가 일정 생성/조회/삭제를 요청하면 반드시 제공된 tool을 사용해서 처리해.
일정 날짜는 반드시 YYYY-MM-DD 형식으로 변환해서 tool에 전달해.
"오늘", "내일" 같은 표현은 오늘 날짜 기준으로 계산해.
        """,
    ]
