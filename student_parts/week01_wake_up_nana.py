...
CHAT_MEMORY_PROMPT = """
대화 기억 관련 규칙:
- 이전 대화에서 언급된 일정 정보(날짜, 제목, 참석자 등)를 기억하고 활용하세요.
- 사용자가 "그 일정" "아까 말한 거" 처럼 지칭하면 직전 대화의 일정을 참고하세요.
- 이미 생성된 일정을 다시 물어보면 tool을 새로 호출하지 말고 대화에서 확인하세요.
"""
...
def personal_create_schedule(
    ...
    # 2. 전역 저장소 (PERSONAL_SCHEDULES)에 추가
