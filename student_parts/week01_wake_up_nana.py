...
CHAT_MEMORY_PROMPT = "이전 대화에서 언급된 일정 정보를 기억하고 맥락에 맞게 활용해."
...
def personal_list_schedules(date_from: str | None = None, date_to: str | None = None) -> str:
    ...
    # 날짜 조건에 맞게 필터링
    if date_from:
        current_schedules = [schedule for schedule in current_schedules if schedule["date"] >= date_from]
    if date_to:
        current_schedules = [schedule for schedule in current_schedules if schedule["date"] <= date_to]

    return _json({"ok": True, "tool_name": "personal_list_schedules", "schedules": current_schedules})
...
def personal_delete_schedule(schedule_id: str) -> str:
    ...
    # 일정 삭제 전 길이 체크
    count_before_delete = len(PERSONAL_SCHEDULES)

    # 리스트 컴프리헨션으로 일정 제거 후 남은 일정만 남겨두기
    # = 재할당 대신 [:]로 같은 리스트 객체의 내용만 교체해 다른 참조가 끊기지 않도록 함.
    PERSONAL_SCHEDULES[:] = [
        schedule for schedule in PERSONAL_SCHEDULES if _schedule_scope(schedule) != current_session_scope() or schedule["id"] != schedule_id
    ]

    # 일정 삭제 후 길이 체크해서 삭제 여부 판단
    deleted = len(PERSONAL_SCHEDULES) < count_before_delete
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        CHAT_MEMORY_PROMPT,
        "너는 Nana라는 개인 일정 관리 비서야.",
        f"오늘의 날짜는 {current_app_date_iso()}이야. 오늘/내일/모레/이번 주/다음 주 등의 표현은 반드시 이 날짜를 기준으로 계산해.",
        "일정 생성/조회/삭제 요청은 반드시 tool을 사용해. tool을 사용하지 않고 임의로 답변하지마."
    ]
