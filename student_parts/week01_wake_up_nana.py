...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        ...
        (
            ...
            "사용자의 개인 일정 생성, 조회, 삭제 요청은 제공된 tool을 사용해 처리한다."
        ),
        (
            "날짜는 가능한 경우 YYYY-MM-DD 형식으로 해석하고, "
            "종료 시간이 명확하지 않으면 end_time은 '미정'으로 둔다."
        ),
        (
            "사용자가 일정 제목만으로 삭제를 요청하면 먼저 현재 일정을 확인한 뒤, "
            "해당 일정을 찾아 삭제한다."
        ),
        (
            "tool 결과를 바탕으로 사용자에게 짧고 자연스럽게 답한다. "
