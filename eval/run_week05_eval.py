"""Week5 tool 라우팅 평가: 케이스별로 실제 agent를 돌려 기대한 tool이 호출됐는지 확인한다.
...
CASES_PATH = Path(__file__).resolve().with_name("week05_cases.json")
...
def run_case(case: dict, index: int) -> dict:
    conversation_id = f"eval_week05_{case['id']}_{index}"
    messages = [{"role": "user", "content": case["query"]}]
    with conversation_session_scope(conversation_id):
        result = run_active_week_agent(5, messages)
